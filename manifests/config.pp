    ...
    case $db_type {
      ...
      'pgsql': {
        exec { 'import schema':
          ...
          command     => "PGPASSWORD=${db_password} psql -h '${db_host}' -U '${db_username}' -d '${db_name}' -w -f ${schema_dir}/pgsql.schema.sql",
          unless      => "PGPASSWORD=${db_password} psql -h '${db_host}' -U '${db_username}' -d '${db_name}' -w -c 'SELECT 1 FROM icingaweb_user'",
          notify      => Exec['create default user'],
          provider    => shell,
        }
        ...
        exec { 'create default user':
          #environment => ["PGPASSWORD=${db_password}"],
          command     => "PGPASSWORD=${db_password} psql -h '${db_host}' -U '${db_username}' -d '${db_name}' -w -c \"INSERT INTO icingaweb_user(name, active, password_hash) VALUES ('icinga', 1, '\\\$1\\\$3no6eqZp\\\$FlcHQDdnxGPqKadmfVcCU.')\"",
          refreshonly => true,
          provider    => shell,
        }
      }
