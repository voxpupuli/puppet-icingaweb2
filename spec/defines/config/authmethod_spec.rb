  ...
  on_supported_os.each do |os, facts|
    context "on #{os}" do
      ...
      context "#{os} with backend 'external'" do
        ...
        it {
          is_expected.to contain_icingaweb2__inisection('authmethod-myauthmethod')
      ...
      context "#{os} with invalid backend" do
        ...
        it { is_expected.to raise_error(Puppet::Error, %r{expects a match for Enum\['db', 'external', 'ldap', 'msldap'\]}) }
      end
    end
  end
end
